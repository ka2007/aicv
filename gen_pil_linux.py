...
def point_zeros(img):
    ...
    point_tl_y = int(sum(point_tl_y) / len(point_tl_y))
    point_tr_x = int(sum(point_tr_x) / len(point_tr_x))
    point_tr_y = int(sum(point_tr_y) / len(point_tr_y))
    point_br_x = int(sum(point_br_x) / len(point_br_x))
    point_br_y = int(sum(point_br_y) / len(point_br_y))
    point_bl_x = int(sum(point_bl_x) / len(point_bl_x))
    point_bl_y = int(sum(point_bl_y) / len(point_bl_y))
...
def rotation(img, pts, rotation_min=-10, rotation_max=10):
    '''
...
def scale(img, pts, scale_min=80, scale_max=150):
    '''
...
def add_background(image, back_image, size_x, size_y, name, path_out, four_zero):
    ...
    # x_scale = np.random.randint(size_x*0.95, size_y*0.99)
    # y_scale = np.random.randint(size_x*0.95, size_y*0.99)
    ...
    out_fn = path_out + 'image' + os.sep + 'temp/{}.jpg'.format(name)
...
def main(process_num=2,fold_num=0,nm=str(os.getpid()) + chr(np.random.randint(97, 122)),rotation_key=False, scale_key=False, transform_key=False, transform_cycle=True, adjustlevel_key=True,
         back_ground_key=True,
         size_x=128, size_y=128, add=0):
    ...
    #
    # path = '/media/aicv/a2acc966-9fe8-4bb3-b64e-9806851a8590/100w/comb_7650/'
    # path_out = '/media/aicv/a2acc966-9fe8-4bb3-b64e-9806851a8590/100w/out_text/'
    # back_path = '/media/aicv/a2acc966-9fe8-4bb3-b64e-9806851a8590/100w/background/'
    # re_point_path = '/media/aicv/a2acc966-9fe8-4bb3-b64e-9806851a8590/100w/re1.txt'
    path = r'F:\data\kingsoft\comb_7650/'
    path_out = 'F:/data/kingsoft/out/'
    back_path = 'F:/data/kingsoft/background/'
    re_point_path = r'F:\data\kingsoft/re1.txt'
    #
    # path = '/root/lixin/gen_data/id_card/comb_7650/'
    # path_out = '/root/lixin/gen_data/id_card/'
    # back_path = '/root/lixin/gen_data/id_card/background/'
    # re_point_path = '/root/lixin/gen_data/id_card/re1.txt'

    # path = '/home/share/lx/id_card/comb_7650/'
    # path_out = '/home/share/lx/id_card/'
    # back_path = '/home/share/lx/id_card/background/'
    # re_point_path = '/home/share/lx/id_card/re1.txt'

    re_point = np.loadtxt(re_point_path, delimiter=" ")
    re_point = re_point[fold_num * len(re_point) / process_num + add:(fold_num + 1) * (len(re_point) / process_num)]
    n = len(re_point)
  
    nm = str(os.getpid())
    # path = 'F:/data/kingsoft/lhl_sfz1.png'
    ...
    for k in range(n):
        ff = open(path_out + 'text/temp/1234_100w.txt', 'a')
        # try:
        ...
        ff.write(nm + '_' + str(count) + '.jpg ' + str(four_points_zero[0][0]) + ' ' + str(
            four_points_zero[0][1]) + ' ' + str(four_points_zero[1][0]) + ' ' + str(
            four_points_zero[1][1]) + ' ' + str(
            four_points_zero[2][0]) + ' ' + str(four_points_zero[2][1]) + ' ' + str(
            four_points_zero[3][0]) + ' ' + str(
...
if __name__ == '__main__':
    ...
    # # parser.add_argument('-x', '--size_x', help='out_size_x')
    # # parser.add_argument('-y', '--size_y', help='out_size_y')
    # parser.add_argument('-f', '--fold_num', help='num ')
    # parser.add_argument('-a', '--add', help='add num ')
    # # parser.add_argument('-nm', '--nm', help='name image ')
    # # parser.add_argument('abc', help='out_size_X')
    # args = parser.parse_args()
    # print args.fold_num, args.add
    # main(fold_num=int(args.fold_num), add=int(args.add))  # arg input str change int

    cores = 2
    for a in range(cores):
        #name= str(time.time())+ chr(np.random.randint(97, 122))
        time.sleep(2)
        p = multiprocessing.Process(target=main, args=(cores,a))
        p.start()
        print "p.pid:", p.pid
        print "p.name:", p.name
